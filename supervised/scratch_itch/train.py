...
import sys
sys.path.append("../../")
from TCN.scratch_itch.model import TCN
from TCN.scratch_itch.utils import data_loader
import numpy as np
...
print(args)
input_size = 7
X_train, X_valid, X_test = data_loader(args.data)
...
model = TCN(input_size, 2, n_channels, kernel_size, dropout=args.dropout)
...
def evaluate(X_data, name='Eval'):
    ...
    with torch.no_grad():
        for idx in eval_idx_list:
            ...
            x, y = Variable(data_line[1:]), Variable([data_line[1]]*len(data_line-1))
            if args.cuda:
                x, y = x.cuda(), y.cuda()
            output = model(x.unsqueeze(0)).squeeze(0)
            loss = -torch.trace(torch.matmul(output-y, output-y))
...
if __name__ == "__main__":
    best_vloss = 1e8
    vloss_list = []
    model_name = "{}.pt".format(args.data)
