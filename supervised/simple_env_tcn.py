...
X_train, Y_train = X_train[:,:-1,:], np.tile(X_train[:,-1,:3],(1,len(X_train)-1,1))
...
model = TCN(input_size, 3, n_channels, kernel_size, dropout=args.dropout)
...
def evaluate(X_data, name='Eval'):
    ...
    with torch.no_grad():
        for idx in eval_idx_list:
            ...
            x, y = torch.Tensor(data_line[1:]), torch.Tensor([data_line[0][:2]]*(len(data_line)-1))
            if args.cuda:
                x, y = x.cuda(), y.cuda()
            output = model(x.unsqueeze(0)).squeeze(0)
            loss = torch.trace(torch.matmul(output-y, (output-y).t()))
