...
class VelocityControl(gym.Env):
  ...
  def _set_obstacles(self):
    ...
    for _i in range(self.NUM_OBSTACLES):
      rad,angle,prad,pangle = random.uniform(*self.OBSTACLE_WIDTH/2),\
        np.pi*random.random(), .5*random.random(), np.pi*random.random()
      center = self.goal + prad*np.array([np.cos(pangle),np.sin(pangle)])
      endpoints = rad*np.array([np.cos(angle),np.sin(angle)]),rad*np.array([np.cos(angle-np.pi),np.sin(angle-np.pi)])
      segments.append((endpoints[0]+center,endpoints[1]+center))
  ...
  def _check_collision(self):
    ...
    for segment in self.segments:
      d,f = segment[1]-segment[0], self.pos-segment[0]
      a,b,c = d.dot(d),2*f.dot(d),f.dot(f)-self.RAD**2
