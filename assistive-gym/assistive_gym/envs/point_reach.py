...
class PointReachEnv(AssistiveEnv):
    def __init__(self, curriculum=False, always_reset=True, robot_type='jaco', success_dist=.05,session_goal=False, frame_skip=5,
        ...
        self.observation_space = spaces.Box(-np.inf, np.inf, (7 + 1 + 7,), dtype=np.float32)  # TODO: observation space size
        self.num_targets = 1
        self.success_dist = .1 if curriculum else .05
        self.debug = debug
        self.stochastic = stochastic
        self.goal_feat = ['target_pos', ]  # Just an FYI
        # self.feature_sizes = OrderedDict({'goal': 6})
    ...
    def step(self, action):
        ...
        sub_goal = self.target_pos if self.target1_reached else self.org_bottle_pos
        info = {
            ...
            'sub_goal': sub_goal,
            'sub_goal_reached': self.target1_reached + self.task_success > self.sub_goal_total,
            'sub_goal_total': self.sub_goal_total
        }
    ...
    def _get_obs(self, forces):
        ...
        robot_obs = dict(
            raw_obs=np.concatenate([self.tool_pos, self.tool_orient, [self.target1_reached], robot_joint_positions]),
    ...
    def reset(self):
        if self.reset_environment or self.always_reset:
            obs = self._reset()
            self.reset_environment = False
            self.sub_goal_total = 0
            return obs
        else:
            self.sub_goal_total = self.target1_reached + self.task_success
            target_color = [0, 0, 1, 1]
            p.changeVisualShape(self.target, -1, rgbaColor=target_color)
            self.curr_step = 0
            return self._get_obs([0])

    def set_reset_environment(self):
        self.reset_environment = True
    ...
    def reset_noise(self):
        ...
        self.bottle_noise = table_boundary * self.np_random.uniform(-1, 1, 3)
        self.goal_noise = table_boundary * self.np_random.uniform(-1, 1, 3)
        while norm(self.bottle_noise - self.goal_noise) < .1:
            self.bottle_noise = table_boundary * self.np_random.uniform(-1, 1, 3)
    ...
    def generate_target(self):
        ...
        bottle_pos = self.org_bottle_pos = self.bottle_noise + table_center - bottle_offset
