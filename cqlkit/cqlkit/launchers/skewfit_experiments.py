...
from cqlkit.envs.vae_wrapper import temporary_mode

import cv2
import numpy as np
import os.path as osp

from cqlkit.samplers.data_collector.vae_env import (
    VAEWrappedEnvPathCollector,
)
from cqlkit.torch.her.her import HERTrainer
from cqlkit.torch.sac.policies import MakeDeterministic
from cqlkit.torch.sac.sac import SACTrainer
from cqlkit.torch.skewfit.online_vae_algorithm import OnlineVaeAlgorithm
from cqlkit.util.io import load_local_or_remote_file
from cqlkit.util.video import dump_video
...
def train_vae(variant, return_data=False):
    from cqlkit.util.ml_util import PiecewiseLinearSchedule
    from cqlkit.torch.vae.conv_vae import (
        ConvVAE,
    )
    import cqlkit.torch.vae.conv_vae as conv_vae
    from cqlkit.torch.vae.vae_trainer import ConvVAETrainer
    from cqlkit.core import logger
    import cqlkit.torch.pytorch_util as ptu
    from cqlkit.pythonplusplus import identity
    import torch
    beta = variant["beta"]
    ...
    if dataset_path is not None:
        ...
        if use_cached and osp.isfile(filename):
            ...
            if random_rollout_data:
                from cqlkit.exploration_strategies.ou_strategy import OUStrategy
...
def get_envs(variant):
    from multiworld.core.image_env import ImageEnv
    from cqlkit.envs.vae_wrapper import VAEWrappedEnv
    from cqlkit.util.io import load_local_or_remote_file
...
def get_exploration_strategy(variant, env):
    from cqlkit.exploration_strategies.epsilon_greedy import EpsilonGreedy
    from cqlkit.exploration_strategies.gaussian_strategy import GaussianStrategy
    from cqlkit.exploration_strategies.ou_strategy import OUStrategy
...
def skewfit_experiment(variant):
    import cqlkit.torch.pytorch_util as ptu
    from cqlkit.data_management.online_vae_replay_buffer import \
        OnlineVaeRelabelingBuffer
    from cqlkit.torch.networks import FlattenMlp
    from cqlkit.torch.sac.policies import TanhGaussianPolicy
    from cqlkit.torch.vae.vae_trainer import ConvVAETrainer
