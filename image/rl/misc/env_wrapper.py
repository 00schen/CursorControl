...
def default_overhead(config):
    ...
    class Overhead(wrapper):
        ...
        def step(self, action):
            ...
            obs['raw_obs'] = np.concatenate([np.array([obs[feat]]).ravel() for feat in self.feature_sizes.keys()])
...
class ground_truth(Adapter):
    """
    Sets goal features to ground truth goal
    """
    def __init__(self, master_env, config):
        super().__init__(master_env,config)
        master_env.goal_feat_sizes = OrderedDict({'ground_truth': master_env.base_goal_size})
...
class sim_target(Adapter):
    def __init__(self, master_env, config):
        ...
        self.target_size = master_env.base_goal_size
