...
from rl.misc.rope_decoder import RopeNeuralProcessMlp, RopeDecoder, RopeConcatTanhGaussianPolicy
...
def experiment(variant):
    ...
    set_dim = 3*env.base_env.num_links
    M = variant["layer_size"]

    if not variant['from_pretrain']:
        qf1 = RopeDecoder(
            rope_latent_size=variant['rope_latent_size'],
            rope_input_indices=[-set_dim],
            rope_input_size=3,
            other_obs_input_size=obs_dim + action_dim,
            output_size=1,
            hidden_sizes=[M, M],
        )
        qf2 = RopeDecoder(
        ...
        policy = RopeConcatTanhGaussianPolicy(
            rope_latent_size=variant['rope_latent_size'],
            rope_input_indices=[-set_dim],
            rope_input_size=3,
            other_obs_input_size=variant['latent_size'],
            output_size=action_dim,
            hidden_sizes=[M, M],
        )
        vae = VAE(input_size=3,
                  latent_size=variant['latent_size'],
                  encoder_hidden_sizes=[64],
                  decoder_hidden_sizes=[64],
                  encoder_class=RopeNeuralProcessMlp
                  ).to(ptu.device)
    else:
...
if __name__ == "__main__":
    ...
    parser.add_argument('--env_name', default='Rope')
    parser.add_argument('--exp_name', default='pretrain_sac_pointreach')
    ...
    search_space = {
        ...
        'latent_size': [4,8],
        'env_config.reward_type': ['rope_exp']
    }
