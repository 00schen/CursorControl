...
calibration_points = [
						(width / 2 + 250, height * 5 / 6),
						(width / 2 + 250, height / 6),
						(width / 2 - 250, height * 5 / 6),
						(width / 2 - 250, height / 6), 
...
while running and sum(remaining) > 0:
	...
	# curr_coord = [np.random.randint(0,width),np.random.randint(0,height)]
	...
	if curr_point is not None:
		draw_circle_with_text(str(curr_point), (0, 0, 0), curr_coord,
							  20, (255, 255, 255))
	# screen.fill((0, 0, 0))
...
dataset = h5py.File(os.path.join(main_dir,'gaze_capture','gaze_data',f'bottle_gaze_data_{current_time}.h5'))
