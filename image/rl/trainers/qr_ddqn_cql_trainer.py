...
class QRDDQNCQLTrainer(DDQNCQLTrainer):
    ...
    def train_from_torch(self, batch):
        ...
        actions = th.argmax(actions, dim=1)
        y_pred = th.stack([curr_logits[i].index_select(0, actions[i]) for i in range(batch_size)]).squeeze(1).unsqueeze(2)
        ...
        #                                                                             - 0.5 * self.kappa)
        huber_loss = th.nn.SmoothL1Loss(beta=self.kappa, reduction='none')(y_pred, y_target)
        tau_hat = (th.arange(n_atoms).to(ptu.device) + 0.5) / n_atoms
        quantile_huber_loss = th.abs(tau_hat[None, :, None] - bellman_errors.le(0.).float()) * huber_loss
        ...
        l2_reg = 0
        # for param in self.qf.gaze_encoder.parameters():
