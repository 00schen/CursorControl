...
class EncDecCQLTrainer(DQNTrainer):
	...
	def train_from_torch(self, batch):
		...
		Supervised loss
		"""
		if 'sup_curr_goal' in batch:
			sup_inputs = batch['sup_curr_gaze_features']
			sup_gt_goal = batch['sup_curr_goal']
			sup_pred_goal = self.encoder(sup_inputs)
			supervised_loss = F.mse_loss(sup_pred_goal,sup_gt_goal)
			loss += supervised_loss
			loss_term_added = True
