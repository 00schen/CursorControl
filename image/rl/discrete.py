...
if __name__ == "__main__":
	variant = dict(
		...
		path_loader_kwargs=dict(
			...
			# ],
			demo_paths=[dict(
					path=os.path.join(os.path.abspath(''),f"demos/LightSwitch_user_model_1000.npy"),
					obs_dict=False,
					is_demo=False,
					train_split=0.9,
				)],
	...
	wrapper_class = target_factory(cap_factory(feedback_factory(default_class(args.env_name))))
	variant.update(dict(
		env_class=railrl_class(wrapper_class,[cap_adapt,window_adapt,target_adapt,]),
	...
	search_space = {
		...
		'trainer_kwargs.learning_rate': [3e-4,1e-3],
		'trainer_kwargs.soft_target_tau': [5e-3,],
		'num_pf_trains_per_train_loop': [250,],
