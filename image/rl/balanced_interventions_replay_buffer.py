...
class BalancedInterReplayBuffer(EnvReplayBuffer):
	...
	def terminate_episode(self):
		...
		self.noop_indices = np.arange(self._size)[self._env_infos['episode_success'][:self._size].flatten().astype(bool)]
		self.inter_indices = np.arange(self._size)[self._env_infos['episode_success'][:self._size].flatten().astype(bool) != True]
