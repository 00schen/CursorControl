...
def make_LSTM1(input_shape,dropout=0,recurrent_dropout=0,num_layer=2,channels=64,kernel_size=1):
    """label prediction same parents as target prediction"""
    episode_input = tf.keras.Input(shape=input_shape,name='episode')
    x = LSTM(channels,dropout=dropout,recurrent_dropout=recurrent_dropout,return_sequences=True)(episode_input)
    for _i in range(num_layer-1):
        x = LSTM(channels,dropout=dropout,recurrent_dropout=recurrent_dropout,return_sequences=True)(x)
    target_pred = Dense(3,name='target')(x)
    label_pred = Dense(2,activation='softmax',name='label')(x)
    model = tf.keras.Model(inputs=[episode_input],outputs=[target_pred,label_pred])
    return model

def make_LSTM2(input_shape,dropout=0,recurrent_dropout=0,num_layer=2,channels=64,kernel_size=1):
    """label prediction same parents as target prediction"""
    episode_input = tf.keras.Input(shape=input_shape,name='episode')
    x = LSTM(channels,dropout=dropout,recurrent_dropout=recurrent_dropout,return_sequences=True)(episode_input)
    for _i in range(num_layer-1):
        x = LSTM(channels,dropout=dropout,recurrent_dropout=recurrent_dropout,return_sequences=True)(x)
    target_pred = Dense(3,name='target')(x)
    label_pred = Dense(2,activation='softmax',name='label')(target_pred)
    model = tf.keras.Model(inputs=[episode_input],outputs=[target_pred,label_pred])
    return model
