...
file_path = os.path.join(dirname,'samples','FeedingJaco-v0')
...
noise = Noise(spaces.Box(low=-.01*np.ones(3),high=.01*np.ones(3)),3,batch=act.shape[1])
...
X = np.concatenate((data['obs'][...,:7],data['obs'][...,10:],act),axis=2)
np.savez_compressed(os.path.join(dirname,'samples','f.noised_trajectory'),X=X,Y=data['target'])
