...
class Data:
    def __init__(self,X,Y,C,split):
        idx = np.random.choice(range(len(X)),int(split*len(X)),replace=False)
        X_train,Y_train,C_train = np.delete(X,idx,axis=0),np.delete(Y,idx,axis=0),np.delete(C,idx,axis=0)
        X_valid,Y_valid,C_valid = X[idx],Y[idx],C[idx]
        train,valid = {'X':X_train,'Y':Y_train,'C':C_train},{'X':X_valid,'Y':Y_valid,'C':C_valid}
    ...
    def update(self,X,Y,C):
        ...
        for old_data,new_data in zip(self.data.values(),[train,valid]):
            ...
            old_data['C'] = np.vstack((old_data['C'],new_data['C']))
...
def serve_data(file,size):
    ...
    C = np.repeat(C[:,np.newaxis],100,1)
    C = tf.one_hot(C.astype(np.int32),2).numpy()
...
class Predictor:
    def __init__(self,env_name,predictor=None,custom_objects=None,oracle_type='target'):
        ...
        if oracle_type == 'target':
            obs_stats,target_stats = np.load(os.path.join(dirname,'data',env,'stats1.npz')).values()
        elif oracle_type == 'trajectory':
    ...
    def predict(self,obs):
        ...
        prediction = self.model.predict(np.array(self.buffer)[np.newaxis,...])
        if len(prediction) == 2:
            target,label = prediction
            target,label = target[0][-1],label[0][-1]
            target = self.target_norm.inverse(target)
        else:
            label = prediction[0][-1]
            target = [-0.32119506, -0.19763412, 0.74035324] if label[0] > .5 else [-0.27993492, 0.06497009, 0.81027984]
        self.label_prediction = np.argmax(label)
