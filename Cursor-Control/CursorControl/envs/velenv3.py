...
class VelocityControl(gym.Env):
  ...
  def __init__(self, max_ep_len=100, oracle_noise=.05, gamma=.9, rollout=3, penalty=10, oracle_dim=2):
    ...
    self.observation_space = spaces.Box(np.array([0]*(3*self.N+self.NUM_OBSTACLES*4+2)),
    np.array([1]*(3*self.N+self.NUM_OBSTACLES*4+2)))
  ...
  def step(self, action):
    ...
    rollout_obs = np.concatenate((self.obs_buffer.flatten(),self.goal))
    ...
    return np.concatenate((rollout_obs,self.obstacles.flatten())), r, done, info
  ...
  def reset(self):
    ...
    return np.concatenate((self.obs_buffer.flatten(),self.goal,self.obstacles.flatten()))
